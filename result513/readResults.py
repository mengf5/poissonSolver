...
if __name__ == "__main__":
    ...
    # close file
    file.close()
#    print '%5s %5s %5s %6s %6s %9s %10s %10s %10s %10s' % (
#        "Nx", "Ny", "Ngp", 
#        "nIter", "nRanks", "nThreads", 
#        "tInit", "tCalc", "tComm", "tbatch")
    for row in range(len(data)):
        print '%5s %5s %5s %6s %6s %9s %10s %10s %10s %10s' % (
        "Nx", "Ny", "Ngp", 
        "nIter", "nRanks", "nThreads", 
        "tInit", "tCalc", "tComm", "tbatch")
        print '%5d %5d %5d %6d %6d %9d %10.3e %10.3e %10.3e %10.3e' % (
            data[row][0],data[row][1],data[row][2],
            data[row][3],data[row][4],data[row][5],
            data[row][9]*freq,data[row][10]*freq,data[row][11]*freq,
            float(data[row][12])*freq)
